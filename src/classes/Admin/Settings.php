...
class Settings {
	 ...
	 * @var ?StringManager
	 */
	private ?StringManager $string_manager = null;
	...
	public function register_settings() {
		...
		add_settings_field(
			...
			[ $this, 'render_password_attempts_limit_field' ],
			self::PAGE_SLUG,
			'ppe_general_section'
		);
		...
		add_settings_section(
			...
			[ $this, 'render_appearance_section_description' ],
			self::PAGE_SLUG
		);
		...
		add_settings_field(
			...
			[ $this, 'render_block_styles_field' ],
			self::PAGE_SLUG,
			'ppe_appearance_section'
		);
		...
		add_settings_section(
			...
			[ $this, 'render_color_customization_section_description' ],
			self::PAGE_SLUG
		);
		...
		add_settings_field(
			...
			[ $this, 'render_primary_color_hover_field' ],
			self::PAGE_SLUG,
			'ppe_color_customization_section'
		);
		...
		add_settings_field(
			...
			[ $this, 'render_success_color_field' ],
			self::PAGE_SLUG,
			'ppe_color_customization_section'
		);
		...
		add_settings_section(
			...
			[ $this, 'render_messages_section_description' ],
			self::PAGE_SLUG
		);

		// Add fields for each customizable string.
		$customizable_strings = $this->string_manager->get_customizable_strings();

		foreach ( $customizable_strings as $key => $string_config ) {
			add_settings_field(
				$key,
				$string_config['label'],
				[ $this, 'render_string_field' ],
				self::PAGE_SLUG,
				'ppe_messages_section',
				[
					'string_key'    => $key,
		...
		add_settings_field(
			...
			[ $this, 'render_auto_clear_cache_field' ],
			self::PAGE_SLUG,
			'ppe_advanced_section'
		);
	}
	...
	public function render_general_section_description() {
		\printf(
			'<p>%s</p>',
			esc_html__( 'Configure general security and session settings for password protection.', 'password-protect-elite' )
		);
	}
	...
	public function render_appearance_section_description() {
		\printf(
			'<p>%s</p>',
			esc_html__( 'Configure the visual appearance and styling of password protection blocks.', 'password-protect-elite' )
		);
	}
	...
	public function render_messages_section_description() {
		\printf(
			'<p>%s</p>',
			esc_html__( 'Customize the default text strings used in the plugin blocks. These will be used as defaults when creating new blocks.', 'password-protect-elite' )
		);
	}
	...
	public function render_advanced_section_description() {
		\printf(
			'<p>%s</p>',
			esc_html__( 'Advanced configuration options for debugging and performance.', 'password-protect-elite' )
		);
	}
	...
	public function render_session_duration_field() {
		$settings       = get_option( self::SETTINGS_GROUP, [] );
		$duration_value = $settings['session_duration'] ?? 24;
		$duration_unit  = $settings['session_duration_unit'] ?? 'hours';
		?>
	...
	public function render_block_styles_section_description() {
		\printf(
			'<p>%s</p>',
			esc_html__( 'Configure which front-end styles are loaded for the plugin blocks.', 'password-protect-elite' )
		);
	}
	...
	public function render_block_styles_field() {
		$settings = get_option( self::SETTINGS_GROUP, [] );
		$value    = $settings['block_styles_mode'] ?? 'all';
		?>
	...
	public function render_string_field( $args ) {
		...
		$settings      = get_option( self::SETTINGS_GROUP, [] );
		$value         = $settings[ $string_key ] ?? $string_config['default'];
		?>
		<input type="text"
	...
	public function render_color_customization_section_description() {
		\printf(
			'<p>%s</p>',
			esc_html__( 'Customize the colors used in the "All" style mode. These settings only apply when Block Styles Mode is set to "All".', 'password-protect-elite' )
		);
	}
	...
	public function render_primary_color_hover_field() {
		$settings = get_option( self::SETTINGS_GROUP, [] );
		$value    = $settings['primary_color_hover'] ?? '#005177';
		?>
		<input type="color"
	...
	public function render_border_color_field() {
		$settings = get_option( self::SETTINGS_GROUP, [] );
		$value    = $settings['border_color'] ?? '#e1e5e9';
		?>
		<input type="color"
	...
	public function render_background_color_field() {
		$settings = get_option( self::SETTINGS_GROUP, [] );
		$value    = $settings['background_color'] ?? '#fff3cd';
		?>
		<input type="color"
	...
	public function render_error_color_field() {
		$settings = get_option( self::SETTINGS_GROUP, [] );
		$value    = $settings['error_color'] ?? '#f8d7da';
		?>
		<input type="color"
	...
	public function render_additional_settings_section_description() {
		\printf(
			'<p>%s</p>',
			esc_html__( 'Additional configuration options for the plugin.', 'password-protect-elite' )
		);
	}
	...
	public function render_auto_clear_cache_field() {
		$settings = get_option( self::SETTINGS_GROUP, [] );
		$value    = $settings['auto_clear_cache'] ?? true;
		?>
		<label>
			<input type="checkbox"
	...
	public function render_password_attempts_limit_field() {
		$settings = get_option( self::SETTINGS_GROUP, [] );
		$value    = $settings['password_attempts_limit'] ?? 5;
		?>
		<input type="number"
		...
		<p class="description">
			...
			\printf(
				...
				esc_html__( 'Set to 0 to disable lockout feature and allow unlimited attempts.', 'password-protect-elite' )
			);
			?>
		</p>
		<?php
	...
	public function render_appearance_tab() {
		...
		<div class="ppe-tab-content">
			...
			<div id="ppe-color-desc" style="<?php echo ( isset( $settings['block_styles_mode'] ) && 'all' !== $settings['block_styles_mode'] ) ? 'display:none;' : ''; ?>">
				<?php $this->render_color_customization_section_description(); ?>
			</div>

			<table class="form-table" role="presentation">
				<tbody id="ppe-color-fields" style="<?php echo ( isset( $settings['block_styles_mode'] ) && 'all' !== $settings['block_styles_mode'] ) ? 'display:none;' : ''; ?>">
				<tr>
					<th scope="row">
	...
	public function sanitize_settings( $input ) {
		...
		if ( isset( $input['block_styles_mode'] ) ) {
			$allowed_modes = [ 'all', 'essential', 'none' ];
		...
		$sanitized['debug_mode']       = $input['debug_mode'] ?? 0;
		$sanitized['auto_clear_cache'] = $input['auto_clear_cache'] ?? 0;
	...
	public static function get_session_duration_hours() {
		$settings = get_option( self::SETTINGS_GROUP, [] );
		$duration = $settings['session_duration'] ?? 24;
		$unit     = $settings['session_duration_unit'] ?? 'hours';
	...
	public function enqueue_admin_scripts( $hook ) {
		if ( 'ppe_password_group_page_' . self::PAGE_SLUG === $hook ) {
			wp_enqueue_style( 'ppe-settings-css', PPE_PLUGIN_URL . 'assets/admin/css/settings.css', [], PPE_VERSION );
			wp_enqueue_script( 'ppe-settings-js', PPE_PLUGIN_URL . 'assets/admin/js/settings.js', [ 'jquery' ], PPE_VERSION, true );
		}
	}
}
